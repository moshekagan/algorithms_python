...
employees.fillna(value=fillna_rule, inplace=True)
...
print(df.isna())
flt = (df.isna().sum() / df.shape[0] < 0.5)  # % of nan
df2 = df.loc[:, flt]
print(df2)
...
df1 = df1.apply(lambda x: pd.Series([chr(i) for i in x]), axis=0)
df2 = pd.DataFrame(np.random.randint(97, 123, size=[4, 4]), columns=['a', 'b', 'c', 'd'])
df2 = df2.apply(lambda x: pd.Series([chr(i) for i in x]), axis=0)
df3 = pd.DataFrame(np.random.randint(97, 123, size=[4, 4]), columns=['a', 'b', 'c', 'd'])
df3 = df3.apply(lambda x: pd.Series([chr(i) for i in x]), axis=0)
