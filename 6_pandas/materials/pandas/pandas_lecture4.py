...
print(diamonds.loc[:,['carat','depth','table','price']].mean())
print(diamonds.loc[:,['carat','depth','table','price']].agg('mean'))
...
print(grpcut.get_group('Good')[['carat','price']].head() )  # Get Specific Group Columns (enables slicing)
...
grpcut_mean=grpcut[['carat','depth','table','price']].mean()  #  mean values for each numeric column by group.
print(grpcut_mean)
...
print(grp_cut_color["price"].count())
...
# transform
diamonds["norm price"] = grpcut[["price"]].transform(lambda x: x-x.mean())
grpcut[["norm price"]].mean()

# ### Filtering: filter() , query()
grpcut_mean=grpcut[['carat','depth','table','price']].mean()  #  mean values for each numeric column by group.
grpcut_mean=grpcut[['carat','depth','table','price']].agg(np.mean)  #  mean values for each numeric column by group.
print(grpcut.size())
print(grpcut_mean)
print(grpcut[['price']].filter(lambda x: x.mean() > 4400).index)

print(diamonds.loc[grpcut[['price']].filter(lambda x: x.mean() > 4400).index])
